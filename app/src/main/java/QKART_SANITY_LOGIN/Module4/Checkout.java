...
public class Checkout {
    ...
    public Boolean selectAddress(String addressToSelect) {
        try {
             ...
             */
            WebElement parentBox = driver.findElement(By.xpath("//div[contains(@class,'MuiBox-root css-vooagt')]/div[1]"));
            List<WebElement> allBoxes = parentBox.findElements(By.className("not-selected"));

            for (WebElement box : allBoxes) {
                if (box.findElement(By.xpath("//div[@class='address-item not-selected MuiBox-root css-0']/div/p")).getText().replaceAll(" ", "")
